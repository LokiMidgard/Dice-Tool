...
namespace Dice.Tables
{
    ...
    internal class CombinationTable : Table
    {
        ...
        public (WhileManager manager, Table table) GetSeccond(in WhileManager manager)
        {
            return this.seccondTable;
        }

        public CombinationTable((WhileManager manager, Table table) firstTable, (WhileManager manager, Table table) seccondTable)
        {
            this.firstTable = firstTable;
            this.seccondTable = seccondTable;
        }
        ...
        protected override bool InternalContains(IP key, in WhileManager manager)
        {
            ...
            if (cachedTable != null)
                return cachedTable.Contains(key);
            return this.GetFirst(manager).Contains(key) || this.GetSeccond(manager).Contains(key);
        }
