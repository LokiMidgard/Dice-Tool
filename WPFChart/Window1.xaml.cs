...
namespace WPFChart3D
{
    ...
    public partial class Window1 : Window
    {
        ...
        class TestDice : Dice.DiceCalculator<bool, int, int>
        {

            protected override bool RoleCalculation(int taw, int dificulty)
            {
                int d1 = W20;
                int d2 = W20;
                int d3 = W20;
        ...
        public async Task TestSurfacePlot(int nGridNo)
        {
            var dice = new TestDice();

            var progress = new Progress<int>(p => System.Diagnostics.Debug.WriteLine(p));
            const int tawCount = 13;
            const int dificultyCount = 3;
            var erg = await dice.DoIt(Enumerable.Range(0, tawCount).ToArray(), Enumerable.Range(0, dificultyCount).ToArray(), progress, TimeSpan.FromSeconds(2), null, default(TimeSpan));

            var result = erg.Where(x => x.Result)
                            .Select(x => new { Factor = x.Factor, Position = Tuple.Create(x.Item1, x.Item2) })
                            .GroupBy(x => x.Position)
                            .Select(x => new { Position = x.Key, Value = x.Sum(y => y.Factor) }).ToDictionary(x => x.Position, x => x.Value);


            int nXNo = tawCount;
            int nYNo = dificultyCount;
            // 1. set the surface grid
            m_3dChart = new UniformSurfaceChart3D();
            ((UniformSurfaceChart3D)m_3dChart).SetGrid(nXNo, nYNo, 0, nXNo-1, 0, nYNo-1);
